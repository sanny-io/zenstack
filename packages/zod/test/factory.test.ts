...
describe.each([
    ['full', schema],
    ['lite', schemaLite],
] as const)('%s schema', (_target, schemaToUse) => {
    const factory = createSchemaFactory(schemaToUse);
    ...
    describe('SchemaFactory - delegate models', () => {
        ...
        describe('makeModelCreateSchema excludes discriminator', () => {
            it('accepts Video create without discriminator and inherited fields', () => {
                ...
                expect(createSchema.safeParse({ duration: 120, url: 'https://example.com/video.mp4' }).success).toBe(
                    true,
                );
            });
    ...
    describe('SchemaFactory - makeModelSchema with options', () => {
        ...
        describe('select', () => {
            ...
            it('select with nested options on a relation', () => {
                ...
                expect(
                    schema.safeParse({ id: 'u1', posts: [{ title: 'Hello', published: true, id: 'p1' }] }).success,
                ).toBe(false);
            });
            ...
            it('select on Post with author relation (nested include)', () => {
                ...
                expect(
                    schema.safeParse({ id: 'p1', author: { id: 'u1', email: 'a@b.com', username: 'x' } }).success,
                ).toBe(false);
            });
        });
    ...
    describe('type alias', () => {
        ...
        it('supports model validation using `@@validate`', () => {
            const userSchema = factory.makeModelSchema('User');
            let result = userSchema.safeParse({ ...validUser, score: 0 });
            expect(result.success).toBe(true);

            result = userSchema.safeParse({ ...validUser, score: 99.0 });
            expect(result.success).toBe(true);

            result = userSchema.safeParse({ ...validUser, score: 100.0 });
            expect(result.success).toBe(false);

            result = userSchema.safeParse({ ...validUser, score: -1.0 });
            expect(result.success).toBe(false);
        });
        ...
        it('supports `@@validate` attributes', () => {
            ...
            result = contactSchema.safeParse('not-a-contact');
            expect(result.success).toBe(false);
        });
        ...
        it('supports multiple `@@validate` attributes', () => {
            ...
            result = contactSchema.safeParse(99.0);
            expect(result.success).toBe(true);
            expect(result.data).toBe(99.0);

            result = contactSchema.safeParse(100.0);
            expect(result.success).toBe(false);

            result = contactSchema.safeParse(-1.0);
            expect(result.success).toBe(false);
        });
    });
});
